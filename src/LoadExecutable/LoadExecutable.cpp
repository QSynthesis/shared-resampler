...
PVOID WINAPI ImageDirectoryEntryToDataInternal(PVOID Base, BOOLEAN MappedAsImage, ULONG *Size,
                                               DWORD SizeOfHeaders,
...
PVOID WINAPI ImageDirectoryEntryToData32(PVOID Base, BOOLEAN MappedAsImage, USHORT DirectoryEntry,
                                         ULONG *Size, IMAGE_FILE_HEADER *ImageFileHeader,
                                         IMAGE_OPTIONAL_HEADER32 *ImageOptionalHeader);
PVOID WINAPI ImageDirectoryEntryToData64(PVOID Base, BOOLEAN MappedAsImage, USHORT DirectoryEntry,
                                         ULONG *Size, IMAGE_FILE_HEADER *ImageFileHeader,
                                         IMAGE_OPTIONAL_HEADER64 *ImageOptionalHeader);
PVOID WINAPI ImageDirectoryEntryToDataRom(PVOID Base, WORD HeaderMagic, USHORT DirectoryEntry,
                                          ULONG *Size, IMAGE_FILE_HEADER *ImageFileHeader,
                                          IMAGE_ROM_OPTIONAL_HEADER *ImageRomHeaders);
LPCVOID WINAPI ImageDirectoryEntryToDataEx(const PVOID Base, const BOOLEAN MappedAsImage,
                                           const USHORT DirectoryEntry, ULONG *Size);
LPCVOID WINAPI ImageDirectoryEntryToData(PVOID Base, BOOLEAN MappedAsImage, USHORT DirectoryEntry,
                                         PULONG Size);
void ParseIAT(HINSTANCE h);

void LoadExe::INIT_LoadExecutable(void *pfnLoadLibraryA, void *pfnGetCurrentProcess,
                                  void *pfnVirtualProtect, void *pfnGetProcAddress,
...
unsigned long long LoadExe::LoadExecutable(const char *szExecutable) {
...
/*
        ...
        LoadDll - If set, the function calls the LoadLibrary function to load the DLL. Otherwise,
   the function calls the GetModuleHandle function to find the DLL.
...
void *__stdcall LoadExe::SwapExportFunctionAddress(unsigned long hModule, char *FunctionName,
                                                   void *SwappedFunction) {
    ...
    pIED =
        (PIMAGE_EXPORT_DIRECTORY) ((LPBYTE) hModule +
    ...
    for (i = 0; i < pIED->AddressOfFunctions; i++) {
        if (!strcmp(FunctionName, (char *) hModule + Name[i])) {
            Address[Ordinal[i]] = INT_PTR(&Address[Ordinal[i]]) - INT_PTR(SwappedFunction);
...
void __stdcall LoadExe::SwapImportFunctionAddress(unsigned long hModule, char *FunctionName,
                                                  void *SwappedFunction) {

    DWORD ulsize = 0;
    PIMAGE_IMPORT_DESCRIPTOR pImportDesc = (PIMAGE_IMPORT_DESCRIPTOR) ImageDirectoryEntryToData(
        (PVOID) hModule, TRUE, IMAGE_DIRECTORY_ENTRY_IMPORT, &ulsize);
    if (!pImportDesc)
        return;
...
void ParseIAT(HINSTANCE h) {
    ...
    for (; pImportDesc->Name; pImportDesc++) {
        ...
        // Replace current function address with new function address
        for (; pThunk->u1.Function; pThunk++) {
            LoadExe::fnFarProceducre_t pfnNew = 0;
            size_t rva = 0;
#ifdef _WIN64
            if (pThunk->u1.Ordinal & IMAGE_ORDINAL_FLAG64)
#else
...
PVOID WINAPI ImageDirectoryEntryToDataInternal(PVOID Base, BOOLEAN MappedAsImage, ULONG *Size,
    ...
    *(ULONG *) Size = DataDirectory->Size;
    ...
    IMAGE_SECTION_HEADER *pSectionHeaders =
        (IMAGE_SECTION_HEADER *) ((BYTE *) ImageOptionalHeader + SizeOfOptionalHeader);
    for (DWORD i = 0; i < NumberOfSections; i++) {
        ...
        if ((DataDirectory->VirtualAddress >= pSectionHeader->VirtualAddress) &&
            (DataDirectory->VirtualAddress <
             (pSectionHeader->SizeOfRawData + pSectionHeader->VirtualAddress))) {
            return (char *) Base +
                   (DataDirectory->VirtualAddress - pSectionHeader->VirtualAddress) +
...
PVOID WINAPI ImageDirectoryEntryToData32(PVOID Base, BOOLEAN MappedAsImage, USHORT DirectoryEntry,
    ...
    *(ULONG *) Size = NULL;
    ...
    return ImageDirectoryEntryToDataInternal(Base, MappedAsImage, Size,
                                             ImageOptionalHeader->SizeOfHeaders, DataDirectory,
...
PVOID WINAPI ImageDirectoryEntryToDataRom(PVOID Base, WORD HeaderMagic, USHORT DirectoryEntry,
    ...
    *(ULONG *) Size = NULL;
    ...
    IMAGE_SECTION_HEADER *pSectionHeader =
        (IMAGE_SECTION_HEADER *) ((BYTE *) ImageRomHeaders + ImageFileHeader->SizeOfOptionalHeader);
    ...
    for (; j < ImageFileHeader->NumberOfSections; j++, pSectionHeader++) {
        ...
        if (DirectoryEntry == 6 && _stricmp((char *) pSectionHeader->Name, ".rdata") == NULL) {
            *(ULONG *) Size = NULL;
            for (BYTE *i = (BYTE *) Base + pSectionHeader->PointerToRawData + 0xC; *(DWORD *) i;
                 i += 0x1C)
                *Size += 0x1C;
            break;
        }
...
/// <param name="MappedAsImage">If the flag is TRUE, the file is mapped by the system as an image.
/// If this flag is FALSE, the file is mapped as a data file by the MapViewOfFile / ReadFile
/// function</param> <param name="DirectoryEntry">The directory entry to be located</param> <param
/// name="Size">A pointer to a variable that receives the size of the data for the directory entry
/// that is located</param> <returns>If the function succeeds, the return value is a pointer to the
/// data for the directory entry</returns>
LPCVOID WINAPI ImageDirectoryEntryToDataEx(const PVOID Base, const BOOLEAN MappedAsImage,
                                           const USHORT DirectoryEntry, ULONG *Size) {
    *(ULONG *) Size = NULL;
    ...
    IMAGE_NT_HEADERS *ImageNtHeader =
        (PIMAGE_NT_HEADERS) ((LPBYTE) pDosHeader + NtHeaderFileOffset);

    if (pDosHeader->e_magic == IMAGE_DOS_SIGNATURE && NtHeaderFileOffset > 0 &&
        NtHeaderFileOffset < 0x10000000u && ImageNtHeader->Signature == IMAGE_NT_SIGNATURE) {
    ...
    switch (ImageOptionalHeader->Magic) {
        case IMAGE_NT_OPTIONAL_HDR32_MAGIC:
            return ImageDirectoryEntryToData32(Base, MappedAsImage, DirectoryEntry, Size,
                                               ImageFileHeader,
                                               (IMAGE_OPTIONAL_HEADER32 *) ImageOptionalHeader);
        case IMAGE_NT_OPTIONAL_HDR64_MAGIC:
            return ImageDirectoryEntryToData64(Base, MappedAsImage, DirectoryEntry, Size,
                                               ImageFileHeader,
                                               (IMAGE_OPTIONAL_HEADER64 *) ImageOptionalHeader);
        case IMAGE_ROM_OPTIONAL_HDR_MAGIC:
            return ImageDirectoryEntryToDataRom(Base, IMAGE_ROM_OPTIONAL_HDR_MAGIC, DirectoryEntry,
                                                Size, ImageFileHeader,
